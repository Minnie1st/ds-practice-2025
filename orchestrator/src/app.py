...
def checkout():
    ...
    try:
        enqueue_response = order_queue_stub.Enqueue(order_queue.EnqueueRequest(
            order_id=order_id,
            user_name=user_name,
            items=[order_queue.Item(name=item['name'], quantity=item['quantity'], price=item['price']) for item in items],
            shipping_method=shipping_method
        ))
        if not enqueue_response.success:
            logger.error(f"Failed to enqueue order {order_id}: {enqueue_response.message}")
            return {'success': False, 'message': enqueue_response.message, 'order_id': ''}, 400
        logger.info(f"Order {order_id} enqueued successfully")
        return {'success': True, 'message': 'Order placed successfully', 'order_id': order_id}, 200
    except grpc.RpcError as e:
        logger.error(f"Order enqueue error: {e}")
        return {'success': False, 'message': 'Order queue service unavailable', 'order_id': ''}, 500
